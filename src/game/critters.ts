...
function drawHat(g: CanvasRenderingContext2D, d: CritterDesign, w: number, h: number) {
  const hat = d.hat;
  if (!hat || hat === "none") return;
  const top = -h * 0.9;
  const accent = d.accent ?? "#c94b4b";
  ink(g, 5);

  /** cloth band hugging the forehead */
  const band = (fill: string, y = top + h * 0.2, thick = h * 0.11) => {
    g.fillStyle = fill;
    g.beginPath();
    g.moveTo(-w * 0.6, y);
    g.quadraticCurveTo(0, y - h * 0.09, w * 0.6, y);
    g.lineTo(w * 0.6, y + thick);
    g.quadraticCurveTo(0, y + thick - h * 0.07, -w * 0.6, y + thick);
    g.closePath();
    g.fill();
    g.stroke();
  };

  /** rounded dome helmet cap */
  const dome = (fill: string, shade: string, lift = 0) => {
    const gr = g.createLinearGradient(-w * 0.5, top - h * 0.2, w * 0.5, top + h * 0.2);
    gr.addColorStop(0, mix(fill, "#ffffff", 0.35));
    gr.addColorStop(1, shade);
    g.fillStyle = gr;
    g.beginPath();
    g.ellipse(0, top + h * 0.12 - lift, w * 0.66, h * 0.34, 0, Math.PI, Math.PI * 2);
    g.lineTo(w * 0.66, top + h * 0.14 - lift);
    g.lineTo(-w * 0.66, top + h * 0.14 - lift);
    g.closePath();
    g.fill();
    g.stroke();
  ...
  /** cone hat (mage / witch) */
  const cone = (fill: string, brimW: number, tilt: number) => {
    g.save();
    g.translate(0, top + h * 0.12);
    g.rotate(tilt);
    g.fillStyle = mix(fill, "#000000", 0.15);
    g.beginPath();
    g.moveTo(-w * 0.5, 0);
    g.quadraticCurveTo(-w * 0.24, -h * 0.5, w * 0.06, -h * 0.95);
    g.quadraticCurveTo(w * 0.06, -h * 0.4, w * 0.5, 0);
    g.closePath();
    g.fill();
    g.stroke();
    g.fillStyle = fill;
    g.beginPath();
    g.ellipse(0, 0, brimW, h * 0.11, 0, 0, Math.PI * 2);
    g.fill();
    g.stroke();
    g.restore();
  };

  /** hood covering the skull and cheeks */
  const hood = (fill: string) => {
    g.fillStyle = fill;
    g.beginPath();
    g.moveTo(-w * 0.72, top + h * 0.72);
    g.quadraticCurveTo(-w * 0.86, top - h * 0.12, 0, top - h * 0.18);
    g.quadraticCurveTo(w * 0.86, top - h * 0.12, w * 0.72, top + h * 0.72);
    g.quadraticCurveTo(w * 0.4, top + h * 0.38, 0, top + h * 0.4);
    g.quadraticCurveTo(-w * 0.4, top + h * 0.38, -w * 0.72, top + h * 0.72);
    g.closePath();
    g.fill();
    g.stroke();
  };

  const goggleLenses = (glass: string, strap: string) => {
    band(strap, top + h * 0.24, h * 0.09);
    for (const sx of [-1, 1]) {
      g.fillStyle = mix(glass, "#ffffff", 0.25);
      g.beginPath();
      g.arc(sx * w * 0.3, top + h * 0.28, w * 0.19, 0, Math.PI * 2);
      g.fill();
      g.stroke();
      g.save();
      g.globalAlpha = 0.6;
      g.fillStyle = "#fff";
      g.beginPath();
      g.arc(sx * w * 0.3 - w * 0.06, top + h * 0.22, w * 0.06, 0, Math.PI * 2);
      g.fill();
      g.restore();
    }
  };
  ...
  switch (hat) {
    case "bandana": {
      band(accent);
      // knot + trailing tails
      g.fillStyle = mix(accent, "#000000", 0.15);
      g.beginPath();
      g.moveTo(-w * 0.58, top + h * 0.22);
      g.lineTo(-w * 0.96, top + h * 0.44);
      g.lineTo(-w * 0.86, top + h * 0.12);
      g.closePath();
      g.fill();
      g.stroke();
      break;
    ...
    case "boomerang": {
      ...
      g.lineWidth = 10;
      g.beginPath();
      g.moveTo(w * 0.32, top - h * 0.16);
      g.quadraticCurveTo(w * 0.86, top - h * 0.44, w * 0.72, top - h * 0.02);
      g.stroke();
      g.strokeStyle = "#b07b3f";
      g.lineWidth = 6;
      g.beginPath();
      g.moveTo(w * 0.32, top - h * 0.16);
      g.quadraticCurveTo(w * 0.86, top - h * 0.44, w * 0.72, top - h * 0.02);
    ...
    case "armyhelm": {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const y = top - h * 0.06 + Math.sin(a) * R;
        i === 0 ? g.moveTo(x, y) : g.lineTo(x, y);
        const a2 = a + Math.PI / 5;
        g.lineTo(Math.cos(a2) * R * 0.45, top - h * 0.06 + Math.sin(a2) * R * 0.45);
      }
      g.closePath();
      g.fill();
      g.stroke();
    ...
    case "ninjamask": {
      g.fillStyle = "#20202c";
      g.beginPath();
      g.moveTo(-w * 0.7, top + h * 0.68);
      g.quadraticCurveTo(-w * 0.82, top - h * 0.1, 0, top - h * 0.16);
      g.quadraticCurveTo(w * 0.82, top - h * 0.1, w * 0.7, top + h * 0.68);
      g.lineTo(w * 0.66, top + h * 0.9);
      g.quadraticCurveTo(0, top + h * 1.05, -w * 0.66, top + h * 0.9);
      g.closePath();
      g.fill();
      g.stroke();
      // eye slit
      g.fillStyle = mix(d.body, "#ffffff", 0.1);
      g.beginPath();
      g.ellipse(0, top + h * 0.42, w * 0.62, h * 0.14, 0, 0, Math.PI * 2);
      g.fill();
      g.stroke();
      break;
    }
    ...
    case "cultisthood": {
      ...
      g.lineWidth = 3;
      g.beginPath();
      g.arc(w * 0.02, top + h * 0.06, w * 0.16, 0, Math.PI * 2);
      g.stroke();
      for (let i = 0; i < 5; i++) {
        const a1 = -Math.PI / 2 + (i * Math.PI * 4) / 5;
        const a2 = -Math.PI / 2 + ((i + 1) * Math.PI * 4) / 5;
        g.beginPath();
        g.moveTo(w * 0.02 + Math.cos(a1) * w * 0.16, top + h * 0.06 + Math.sin(a1) * w * 0.16);
        g.lineTo(w * 0.02 + Math.cos(a2) * w * 0.16, top + h * 0.06 + Math.sin(a2) * w * 0.16);
        g.stroke();
      }
      ink(g, 5);
      break;
    ...
    case "reaperhood": {
      ...
      g.lineWidth = 9;
      g.beginPath();
      g.moveTo(w * 0.9, top + h * 1.5);
      g.lineTo(w * 0.72, top - h * 0.35);
      g.stroke();
      ...
      g.fillStyle = "#d8dde8";
      g.beginPath();
      g.moveTo(w * 0.72, top - h * 0.35);
      g.quadraticCurveTo(w * 0.1, top - h * 0.5, w * 0.02, top - h * 0.1);
      g.quadraticCurveTo(w * 0.34, top - h * 0.3, w * 0.72, top - h * 0.2);
      g.closePath();
      g.fill();
      g.stroke();
      break;
    ...
    case "piratehat": {
      band(accent, top + h * 0.24, h * 0.09);
      g.fillStyle = "#1d1a26";
      g.beginPath();
      g.moveTo(-w * 0.95, top + h * 0.1);
      g.quadraticCurveTo(0, top - h * 0.62, w * 0.95, top + h * 0.1);
      g.quadraticCurveTo(0, top + h * 0.02, -w * 0.95, top + h * 0.1);
      g.closePath();
      g.fill();
      g.stroke();
      g.fillStyle = "#f4f1ff";
      g.beginPath();
      g.arc(0, top - h * 0.16, w * 0.1, 0, Math.PI * 2);
      g.fill();
      g.fillStyle = "#1d1a26";
      for (const sx of [-1, 1]) {
        g.beginPath();
        g.arc(sx * w * 0.035, top - h * 0.18, w * 0.026, 0, Math.PI * 2);
        g.fill();
      }
      break;
    }
    case "vikinghelm": {
      ...
      for (const sx of [-1, 1]) {
        g.fillStyle = "#f2e6c8";
        g.beginPath();
        g.moveTo(sx * w * 0.58, top + h * 0.06);
        g.quadraticCurveTo(sx * w * 1.05, top - h * 0.1, sx * w * 0.92, top - h * 0.46);
        g.quadraticCurveTo(sx * w * 0.82, top - h * 0.12, sx * w * 0.46, top - h * 0.02);
        g.closePath();
        g.fill();
        g.stroke();
      }
    ...
    case "knighthelm": {
      ...
      g.fillStyle = "#9aa3b3";
      g.beginPath();
      g.rect(-w * 0.5, top + h * 0.14, w, h * 0.5);
      g.fill();
      g.stroke();
      g.strokeStyle = OUTLINE;
      g.lineWidth = 4;
      for (let i = -2; i <= 2; i++) {
        g.beginPath();
        g.moveTo(i * w * 0.18, top + h * 0.18);
        g.lineTo(i * w * 0.18, top + h * 0.6);
        g.stroke();
      }
      ink(g, 5);
      break;
    ...
    case "gladiatorhelm": {
      ...
      g.fillStyle = accent;
      g.beginPath();
      g.moveTo(-w * 0.1, top - h * 0.02);
      g.quadraticCurveTo(0, top - h * 0.56, w * 0.24, top - h * 0.5);
      g.quadraticCurveTo(w * 0.12, top - h * 0.16, w * 0.16, top + h * 0.02);
      g.closePath();
      g.fill();
      g.stroke();
      break;
    ...
    case "feathercap": {
      dome("#5c8b47", "#33562a", h * 0.02);
      g.fillStyle = "#f4f4ee";
      g.beginPath();
      g.moveTo(-w * 0.5, top - h * 0.02);
      g.quadraticCurveTo(-w * 0.9, top - h * 0.42, -w * 0.55, top - h * 0.52);
      g.quadraticCurveTo(-w * 0.46, top - h * 0.2, -w * 0.38, top - h * 0.02);
      g.closePath();
      g.fill();
      g.stroke();
      break;
    ...
    case "wizardhat":
      ...
      for (const [sx, sy] of [
        ...
        g.beginPath();
        g.arc(w * sx, top + h * (0.12 + sy), w * 0.08, 0, Math.PI * 2);
        g.fill();
      }
      break;
    case "witchhat":
      cone("#241f31", w * 0.86, 0.08);
      break;
    case "antlers": {
      g.strokeStyle = OUTLINE;
      for (const sx of [-1, 1]) {
        for (const lw of [11, 6]) {
          g.strokeStyle = lw === 11 ? OUTLINE : "#8a6234";
          g.lineWidth = lw;
          g.beginPath();
          g.moveTo(sx * w * 0.3, top + h * 0.06);
          g.quadraticCurveTo(sx * w * 0.62, top - h * 0.3, sx * w * 0.5, top - h * 0.6);
          g.moveTo(sx * w * 0.46, top - h * 0.24);
          g.lineTo(sx * w * 0.86, top - h * 0.4);
          g.stroke();
        }
      }
      ink(g, 5);
      ...
      for (const sx of [-1, 1]) {
        g.beginPath();
        g.ellipse(sx * w * 0.86, top - h * 0.42, w * 0.1, h * 0.05, sx * 0.5, 0, Math.PI * 2);
        g.fill();
      }
      break;
    }
    case "featherband": {
      ...
      g.fillStyle = "#8fd4ff";
      g.beginPath();
      g.moveTo(w * 0.34, top + h * 0.16);
      g.quadraticCurveTo(w * 0.72, top - h * 0.34, w * 0.42, top - h * 0.5);
      g.quadraticCurveTo(w * 0.32, top - h * 0.16, w * 0.24, top + h * 0.14);
      g.closePath();
      g.fill();
      g.stroke();
      break;
    ...
    case "headband": {
      band(accent);
      g.fillStyle = mix(accent, "#ffffff", 0.7);
      g.beginPath();
      g.arc(0, top + h * 0.22, w * 0.09, 0, Math.PI * 2);
      g.fill();
      g.stroke();
      break;
    }
    case "mitre": {
      g.fillStyle = "#f6f3ea";
      g.beginPath();
      g.moveTo(-w * 0.46, top + h * 0.18);
      g.quadraticCurveTo(-w * 0.42, top - h * 0.5, 0, top - h * 0.72);
      g.quadraticCurveTo(w * 0.42, top - h * 0.5, w * 0.46, top + h * 0.18);
      g.closePath();
      g.fill();
      g.stroke();
      g.strokeStyle = "#e2b33c";
      g.lineWidth = 5;
      g.beginPath();
      g.moveTo(0, top - h * 0.56);
      g.lineTo(0, top - h * 0.06);
      g.moveTo(-w * 0.16, top - h * 0.34);
      g.lineTo(w * 0.16, top - h * 0.34);
      g.stroke();
      ink(g, 5);
      break;
    }
    case "goggles":
      goggleLenses(accent, "#6b5334");
      break;
    case "gasmask": {
      band("#5c6b3a", top + h * 0.18, h * 0.09);
      g.fillStyle = "#4e5a33";
      g.beginPath();
      g.ellipse(0, top + h * 0.52, w * 0.56, h * 0.3, 0, 0, Math.PI * 2);
      g.fill();
      g.stroke();
      g.fillStyle = "#9fd06a";
      for (const sx of [-1, 1]) {
        g.beginPath();
        g.arc(sx * w * 0.26, top + h * 0.44, w * 0.16, 0, Math.PI * 2);
        g.fill();
        g.stroke();
      }
      g.fillStyle = "#c8a03c";
      g.beginPath();
      g.arc(0, top + h * 0.72, w * 0.15, 0, Math.PI * 2);
      g.fill();
      g.stroke();
      break;
    }
    case "headmirror": {
      band("#e7e4dc", top + h * 0.14, h * 0.09);
      g.fillStyle = "#f7f7f7";
      g.beginPath();
      g.arc(-w * 0.02, top + h * 0.06, w * 0.19, 0, Math.PI * 2);
      g.fill();
      g.stroke();
      g.fillStyle = OUTLINE;
      g.beginPath();
      g.arc(-w * 0.02, top + h * 0.06, w * 0.06, 0, Math.PI * 2);
      g.fill();
      break;
    }
    case "mohawk": {
      g.fillStyle = accent;
      g.beginPath();
      g.moveTo(-w * 0.34, top + h * 0.14);
      for (let i = 0; i <= 5; i++) {
        const t = i / 5;
        const x = -w * 0.34 + t * w * 0.68;
        g.lineTo(x, top - h * (0.28 + Math.sin(t * Math.PI) * 0.22));
        g.lineTo(x + w * 0.06, top + h * 0.06);
      }
      g.lineTo(w * 0.34, top + h * 0.14);
      g.closePath();
      g.fill();
      g.stroke();
      break;
    ...
    case "jesterhat": {
      const lobe = (sx: number, fill: string) => {
        g.fillStyle = fill;
        g.beginPath();
        g.moveTo(0, top + h * 0.12);
        g.quadraticCurveTo(sx * w * 0.3, top - h * 0.6, sx * w * 0.86, top - h * 0.42);
        g.quadraticCurveTo(sx * w * 0.4, top - h * 0.16, sx * w * 0.5, top + h * 0.16);
        g.closePath();
        g.fill();
        g.stroke();
        g.fillStyle = "#ffd85e";
        g.beginPath();
        g.arc(sx * w * 0.9, top - h * 0.38, w * 0.1, 0, Math.PI * 2);
        g.fill();
        g.stroke();
      };
      lobe(-1, "#cf3f46");
    ...
    case "afro": {
      const cols = ["#e14b4b", "#e8973c", "#f0d34a", "#5fbf5a", "#4a8fe0", "#8a5bd6"];
      cols.forEach((c, i) => {
        const a = Math.PI + (i / (cols.length - 1)) * Math.PI;
        g.fillStyle = c;
        g.beginPath();
        g.arc(Math.cos(a) * w * 0.58, top + h * 0.14 + Math.sin(a) * h * 0.3, w * 0.26, 0, Math.PI * 2);
        g.fill();
        g.stroke();
      });
      g.fillStyle = "#e3453f";
      g.beginPath();
      g.arc(0, top + h * 0.62, w * 0.13, 0, Math.PI * 2);
      g.fill();
      g.stroke();
      break;
    }
    case "sheet": {
      g.fillStyle = "#f4f3fb";
      g.beginPath();
      g.moveTo(-w * 0.8, top + h * 1.5);
      g.quadraticCurveTo(-w * 0.92, top - h * 0.16, 0, top - h * 0.2);
      g.quadraticCurveTo(w * 0.92, top - h * 0.16, w * 0.8, top + h * 1.5);
      for (let i = 3; i >= 0; i--) {
        const x = -w * 0.8 + (i / 3) * w * 1.6;
        g.quadraticCurveTo(x + w * 0.2, top + h * 1.32, x, top + h * 1.5);
      }
      g.closePath();
      g.fill();
      g.stroke();
      ...
      for (const sx of [-1, 1]) {
        g.beginPath();
        g.ellipse(sx * w * 0.26, top + h * 0.5, w * 0.16, h * 0.13, 0, 0, Math.PI * 2);
        g.fill();
      }
      break;
    }
    case "demonhorns": {
      for (const sx of [-1, 1]) {
        g.fillStyle = mix(accent, "#000000", 0.1);
        g.beginPath();
        g.moveTo(sx * w * 0.3, top + h * 0.1);
        g.quadraticCurveTo(sx * w * 0.66, top - h * 0.12, sx * w * 0.5, top - h * 0.52);
        g.quadraticCurveTo(sx * w * 0.34, top - h * 0.16, sx * w * 0.14, top + h * 0.1);
        g.closePath();
        g.fill();
        g.stroke();
      }
